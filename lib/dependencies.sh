...
npm_node_modules() {
  ...
  if [ -e "$build_dir/package.json" ]; then
    ...
    if [[ "$USE_NPM_INSTALL" == "false" ]]; then
      ...
      monitor "npm-install" npm ci --production="$production" -w @anyroad/medusa --include-workspace-root --unsafe-perm --userconfig "$build_dir/.npmrc" 2>&1
    else
      ...
      monitor "npm-install" npm install --production="$production" -w @anyroad/medusa --unsafe-perm --userconfig "$build_dir/.npmrc" 2>&1
    fi
  else
